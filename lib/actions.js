...
function waitelem(self, selector, done) {
  ...
  var newDone = function(err) {
    if (err) {
      return done(
        new Error(
          `.wait() for ${selector} timed out after ${self.options.waitTimeout}msec`
        )
      )
    }
    done()
...
exports.evaluate = function(fn /**, arg1, arg2..., done**/) {
  ...
  var timeoutTimer = setTimeout(function() {
      ...
      new Error(
        `Evaluation timed out after ${self.options.executionTimeout}msec.  Are you calling done() or resolving your promises?`
      )
    )
