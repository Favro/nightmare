...
app.on('ready', function() {
  ...
  let webContentsListeners = {}

  function installWebContentsListener(webContents, listener) {
    webContentsListeners[webContents.id] = listener

    if (!webContents.session.nightmareWebContentsInstaled) {
      webContents.session.nightmareWebContentsInstaled = true

      let webContentsId = webContents.id
      webContents.session.webRequest.onCompleted(function(details) {
        if (
          details.webContentsId == webContentsId &&
          webContentsListeners[webContentsId]
        ) {
          webContentsListeners[webContentsId](details)
        }
      })
    }
  }

  function removeWebContentsListener(webContents) {
    delete webContentsListeners[webContents.id]
  }
  ...
  parent.respondTo('goto', function(url, headers, timeout, done) {
    ...
    if (win.webContents.getURL() == url) {
      ...
      var timer = setTimeout(function() {
        ...
        // After executeJavaScript race condition was fixed:
        // https://github.com/electron/electron/issues/13504
        // We need to stop loading to allow executeJavaScript to run
        let oldCleanedUp = cleanedUp
        cleanedUp = true
        win.webContents.stop()
        cleanedUp = oldCleanedUp
      ...
      function handleFailure(event, code, detail, failedUrl, isMainFrame) {
        console.log('handleFailure', arguments)
        if (isMainFrame) {
          cleanup({
      ...
      function handleStop() {
        if (expectedFinish > 1 || cleanedUp) return

        cleanup({
          message:
            'navigation error: stopped before receiving success or failure',
          code: 304,
          details: 'stopped before receiving success or failure',
          url: url
        })
      }
      ...
      function handleFinish(_event) {
        ...
        console.log('handleFinish', arguments)
      ...
      function cleanup(err, data) {
        ...
        win.webContents.removeListener('did-stop-loading', handleStop)
        win.webContents.removeListener(
          'did-fail-provisional-load',
          handleFailure
        )
        removeWebContentsListener(win.webContents)
      ...
      function canLoadProtocol(protocol, callback) {
        ...
        callback(
          electron.protocol.isProtocolRegistered(protocol) ||
            electron.protocol.isProtocolIntercepted(protocol)
        )
      }
      ...
      function startLoading() {
        ...
        installWebContentsListener(win.webContents, details => {
  ...
  parent.respondTo('cookie.get', function(query, done) {
    ...
    win.webContents.session.cookies
      ...
      .then(function(cookies) {
        parent.emit('log', 'getting cookie result: ' + JSON.stringify(cookies))
        done(null, details.name ? cookies[0] : cookies)
      })
      .catch(function(err) {
        parent.emit('log', 'getting cookie error: ' + JSON.stringify(err))
        done(err)
      })
  })
  ...
  parent.respondTo('cookie.set', function(cookies, done) {
    ...
    for (var i = 0, cookie; (cookie = cookies[i]); i++) {
      var details = Object.assign(
        {
          url: 'https://lolalo:55:55:55/jolo'
        },
        cookie
      )
    ...
    expirationDate?: number;
      ...
      win.webContents.session.cookies
        ...
        .then(function() {
          parent.emit(
            'log',
            'setting cookie result:' + JSON.stringify(arguments)
          )
          done()
        })
        .catch(function(err) {
          parent.emit(
            'log',
            'setting cookie error: ' + JSON.stringify(arguments)
          )
          done(err)
        })
    }
  ...
  parent.respondTo('cookie.clear', function(cookies, done) {
    ...
    var filter = cookies
    if (cookies.length == 0) filter = { url: url }
